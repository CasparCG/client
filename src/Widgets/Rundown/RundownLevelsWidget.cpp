...
void RundownLevelsWidget::executeStop()
{
    ...
    if (device != NULL && device->isConnected())
        device->setLevels(this->command.channel.get(), this->command.getVideolayer(), 0, 1, 1, 0, 1);
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        if (deviceShadow != NULL && deviceShadow->isConnected())
            deviceShadow->setLevels(this->command.channel.get(), this->command.getVideolayer(), 0, 1, 1, 0, 1);
    }
}
...
void RundownLevelsWidget::executePlay()
{
    const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device != NULL && device->isConnected())
        device->setLevels(this->command.channel.get(), this->command.getVideolayer(), this->command.getMinIn(), this->command.getMaxIn(),
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        const QSharedPointer<CasparDevice>  deviceShadow = DeviceManager::getInstance().getDeviceByName(model.getName());
        if (deviceShadow != NULL && deviceShadow->isConnected())
            deviceShadow->setLevels(this->command.channel.get(), this->command.getVideolayer(), this->command.getMinIn(), this->command.getMaxIn(),
                                    this->command.getGamma(), this->command.getMinOut(), this->command.getMaxOut(), this->command.getTransitionDuration(),
                                    this->command.getTween(), this->command.getDefer());
    }
