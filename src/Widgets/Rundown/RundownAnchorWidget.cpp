...
void RundownAnchorWidget::executeStop()
{
    ...
    if (device != NULL && device->isConnected())
        device->setAnchor(this->command.channel.get(), this->command.getVideolayer(), 0, 0);
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        if (deviceShadow != NULL && deviceShadow->isConnected())
            deviceShadow->setAnchor(this->command.channel.get(), this->command.getVideolayer(), 0, 0);
    }
}
...
void RundownAnchorWidget::executePlay()
{
    const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device != NULL && device->isConnected())
        device->setAnchor(this->command.channel.get(), this->command.getVideolayer(), this->command.getPositionX(),
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        const QSharedPointer<CasparDevice>  deviceShadow = DeviceManager::getInstance().getDeviceByName(model.getName());
        if (deviceShadow != NULL && deviceShadow->isConnected())
            deviceShadow->setAnchor(this->command.channel.get(), this->command.getVideolayer(), this->command.getPositionX(),
                                    this->command.getPositionY(), this->command.getTransitionDuration(), this->command.getTween(), this->command.getDefer());
    }
