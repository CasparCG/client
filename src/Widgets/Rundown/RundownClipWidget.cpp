...
void RundownClipWidget::executeStop()
{
    ...
    if (device != NULL && device->isConnected())
        device->setClipping(this->command.channel.get(), this->command.videolayer.get(), 0, 0, 1, 1);
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        if (deviceShadow != NULL && deviceShadow->isConnected())
            deviceShadow->setClipping(this->command.channel.get(), this->command.videolayer.get(), 0, 0, 1, 1);
    }
}
...
void RundownClipWidget::executePlay()
{
    const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device != NULL && device->isConnected())
        device->setClipping(this->command.channel.get(), this->command.videolayer.get(), this->command.getLeft(),
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        const QSharedPointer<CasparDevice>  deviceShadow = DeviceManager::getInstance().getDeviceByName(model.getName());
        if (deviceShadow != NULL && deviceShadow->isConnected())
            deviceShadow->setClipping(this->command.channel.get(), this->command.videolayer.get(), this->command.getLeft(),
                                      this->command.getTop(), this->command.getWidth(), this->command.getHeight(),
