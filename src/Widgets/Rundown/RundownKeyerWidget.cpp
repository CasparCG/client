...
void RundownKeyerWidget::executeStop()
{
    ...
    if (device != NULL && device->isConnected())
        device->setKeyer(this->command.channel.get(), this->command.getVideolayer(), 0);
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        if (deviceShadow != NULL && deviceShadow->isConnected())
            deviceShadow->setKeyer(this->command.channel.get(), this->command.getVideolayer(), 0);
    }
}
...
void RundownKeyerWidget::executePlay()
{
    const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device != NULL && device->isConnected())
        device->setKeyer(this->command.channel.get(), this->command.getVideolayer(), 1, this->command.getDefer());
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        const QSharedPointer<CasparDevice>  deviceShadow = DeviceManager::getInstance().getDeviceByName(model.getName());
        if (deviceShadow != NULL && deviceShadow->isConnected())
            deviceShadow->setKeyer(this->command.channel.get(), this->command.getVideolayer(), 1, this->command.getDefer());
    }
