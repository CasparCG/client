#include "RundownAtemMacroWidget.h"
...
RundownAtemMacroWidget::RundownAtemMacroWidget(const LibraryModel& model, QWidget* parent, const QString& color, bool active,
...
void RundownAtemMacroWidget::preview(const PreviewEvent& event)
{
    Q_UNUSED(event);
...
void RundownAtemMacroWidget::labelChanged(const LabelChangedEvent& event)
{
    // This event is not for us.
    if (!this->selected)
        return;
...
AbstractRundownWidget* RundownAtemMacroWidget::clone()
{
    RundownAtemMacroWidget* widget = new RundownAtemMacroWidget(this->model, this->parentWidget(), this->color, this->active,
                                                                this->inGroup, this->compactView);

    AtemMacroCommand* command = dynamic_cast<AtemMacroCommand*>(widget->getCommand());
...
void RundownAtemMacroWidget::setCompactView(bool compactView)
{
    if (compactView)
    {
...
void RundownAtemMacroWidget::readProperties(boost::property_tree::wptree& pt)
{
...
bool RundownAtemMacroWidget::isInGroup() const
{
    return this->inGroup;
}

AbstractCommand* RundownAtemMacroWidget::getCommand()
{
    return &this->command;
}

LibraryModel* RundownAtemMacroWidget::getLibraryModel()
{
    return &this->model;
}

void RundownAtemMacroWidget::setSelected(bool selected)
{
    this->selected = selected;
}

void RundownAtemMacroWidget::setActive(bool active)
{
    this->active = active;
...
void RundownAtemMacroWidget::setInGroup(bool inGroup)
{
    this->inGroup = inGroup;
    this->labelGroupColor->setVisible(this->inGroup);
}

QString RundownAtemMacroWidget::getColor() const
{
    return this->color;
}

void RundownAtemMacroWidget::setColor(const QString& color)
{
    this->color = color;
    this->setStyleSheet(QString("#frameItem, #frameStatus { background-color: %1; }").arg(color));
}

void RundownAtemMacroWidget::checkEmptyDevice()
{
...
void RundownAtemMacroWidget::clearDelayedCommands()
{
    this->executeTimer.stop();
}

void RundownAtemMacroWidget::setUsed(bool used)
{
    if (used)
    {
...
bool RundownAtemMacroWidget::executeCommand(Playout::PlayoutType type)
{
...
void RundownAtemMacroWidget::executePlay()
{
    const QSharedPointer<AtemDevice> device = AtemDeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
...
void RundownAtemMacroWidget::delayChanged(int delay)
{
    this->labelDelay->setText(QString("Delay: %1").arg(delay));
}

void RundownAtemMacroWidget::checkGpiConnection()
{
...
void RundownAtemMacroWidget::checkDeviceConnection()
{
    const QSharedPointer<AtemDevice> device = AtemDeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device == NULL)
...
void RundownAtemMacroWidget::configureOscSubscriptions()
{
...
void RundownAtemMacroWidget::allowGpiChanged(bool allowGpi)
{
    Q_UNUSED(allowGpi);
...
void RundownAtemMacroWidget::gpiConnectionStateChanged(bool connected, GpiDevice* device)
{
    Q_UNUSED(connected);
    Q_UNUSED(device);
...
void RundownAtemMacroWidget::remoteTriggerIdChanged(const QString& remoteTriggerId)
{
...
void RundownAtemMacroWidget::deviceConnectionStateChanged(AtemDevice& device)
{
    Q_UNUSED(device);
...
void RundownAtemMacroWidget::deviceAdded(AtemDevice& device)
{
...
void RundownAtemMacroWidget::playControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownAtemMacroWidget::playNowControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
...
void RundownAtemMacroWidget::updateControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
