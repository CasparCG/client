#include "RundownAtemAutoWidget.h"
...
AbstractRundownWidget* RundownAtemAutoWidget::clone()
{
    RundownAtemAutoWidget* widget = new RundownAtemAutoWidget(this->model, this->parentWidget(), this->color, this->active,
...
void RundownAtemAutoWidget::setCompactView(bool compactView)
{
    if (compactView)
    {
...
void RundownAtemAutoWidget::readProperties(boost::property_tree::wptree& pt)
{
    if (pt.count(L"color") > 0) setColor(QString::fromStdWString(pt.get<std::wstring>(L"color")));
}

void RundownAtemAutoWidget::writeProperties(QXmlStreamWriter* writer)
{
    writer->writeTextElement("color", this->color);
}

bool RundownAtemAutoWidget::isGroup() const
{
    return false;
}

bool RundownAtemAutoWidget::isInGroup() const
{
    return this->inGroup;
}

AbstractCommand* RundownAtemAutoWidget::getCommand()
{
    return &this->command;
}
...
void RundownAtemAutoWidget::setInGroup(bool inGroup)
{
    this->inGroup = inGroup;
    this->labelGroupColor->setVisible(this->inGroup);
}

QString RundownAtemAutoWidget::getColor() const
{
    return this->color;
}

void RundownAtemAutoWidget::setColor(const QString& color)
{
    this->color = color;
    this->setStyleSheet(QString("#frameItem, #frameStatus { background-color: %1; }").arg(color));
}

void RundownAtemAutoWidget::checkEmptyDevice()
{
...
void RundownAtemAutoWidget::clearDelayedCommands()
{
    this->executeTimer.stop();
}

void RundownAtemAutoWidget::setUsed(bool used)
{
    if (used)
    {
...
void RundownAtemAutoWidget::checkGpiConnection()
{
...
void RundownAtemAutoWidget::checkDeviceConnection()
{
    const QSharedPointer<AtemDevice> device = AtemDeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device == NULL)
...
void RundownAtemAutoWidget::remoteTriggerIdChanged(const QString& remoteTriggerId)
{
...
void RundownAtemAutoWidget::deviceAdded(AtemDevice& device)
{
