...
void RundownAutoWidget::preview(const PreviewEvent& event)
{
    Q_UNUSED(event);
...
void RundownAutoWidget::labelChanged(const LabelChangedEvent& event)
{
    // This event is not for us.
    if (!this->selected)
        return;
...
void RundownAutoWidget::setSelected(bool selected)
{
...
void RundownAutoWidget::clearDelayedCommands()
{
    this->executeTimer.stop();
}

void RundownAutoWidget::setUsed(bool used)
{
    if (used)
    {
...
bool RundownAutoWidget::executeCommand(Playout::PlayoutType type)
{
...
void RundownAutoWidget::playNowControlSubscriptionReceived(const QString& predicate, const QList<QVariant>& arguments)
{
    Q_UNUSED(predicate);
