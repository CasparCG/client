...
void RundownDeckLinkInputWidget::executePlay()
{
    ...
    if (device != NULL && device->isConnected())
    {
        if (this->loaded)
            ...
            device->playDeviceInput(this->command.channel.get(), this->command.videolayer.get(), this->command.getDevice(),
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        if (deviceShadow != NULL && deviceShadow->isConnected())
        {
            if (this->loaded)
                ...
                deviceShadow->playDeviceInput(this->command.channel.get(), this->command.videolayer.get(), this->command.getDevice(),
                                              this->command.getFormat());
        }
    }
...
void RundownDeckLinkInputWidget::executeLoad()
{
    const QSharedPointer<CasparDevice> device = DeviceManager::getInstance().getDeviceByName(this->model.getDeviceName());
    if (device != NULL && device->isConnected())
        device->loadDeviceInput(this->command.channel.get(), this->command.videolayer.get(), this->command.getDevice(), this->command.getFormat());
    ...
    foreach (const DeviceModel& model, DeviceManager::getInstance().getDeviceModels())
    {
        ...
        const QSharedPointer<CasparDevice>  deviceShadow = DeviceManager::getInstance().getDeviceByName(model.getName());
        if (deviceShadow != NULL && deviceShadow->isConnected())
            deviceShadow->loadDeviceInput(this->command.channel.get(), this->command.videolayer.get(), this->command.getDevice(), this->command.getFormat());
    }
